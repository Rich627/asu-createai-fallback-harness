...
安裝器會：沿用 Keychain 既有的 CreateAI token（沒有才要求輸入一次）、對 `aws/claude5_opus` 與 `aws/claude4_5_haiku` 各跑一次工具往返實測（驗證 opus 與 haiku 兩條對應路徑）、安裝登入時自動啟動的 LaunchAgent（`com.rich.asu-claude-bridge`，固定 `127.0.0.1:41118`），最後才在 `~/.claude/settings.json` 的 `env` 寫入 `ANTHROPIC_BASE_URL`。健康檢查沒過就不會動 `settings.json`。原檔備份為 `~/.claude/settings.json.asu-backup-*`。
...
- 模型對應是自動的：你在 `/model` 選什麼，fallback 就用對應的 CreateAI 模型（`claude-sonnet-5` → `aws/claude5_sonnet`、`claude-haiku-4-5` → `aws/claude4_5_haiku`、`gpt-5.6-sol` → `openai/gpt5_6_sol`）。找不到對應時用同系列最新的，再不行才用預設 `aws/claude5_opus`。想固定成單一模型就傳 `--model <CreateAI id>`。
